  ...
  return (
    <div className="grid col-span-full grid-cols-subgrid">
      <ul className="grid gap-6 grid-cols-subgrid col-span-full">
        {data?.map((item) => <MusicalTypeCard data={item} key={item._id} />)}
      </ul>
    </div>
  )
}
...
function MusicalTypeCard({ data }) {
  ...
  const { setTime, getTime } = useVideoTime()

  useEffect(() => {
    const video = videoRef.current
    if (!video) return

    const savedTime = getTime(data._id)
        ...
        requestAnimationFrame(() => {
          video.play().catch((err) => {
            if (err.name !== 'AbortError') {
              console.warn('Card video.play() failed', err)
            }
          })
        })
      } catch (err) {
        console.warn('Card: setting currentTime failed', err)
      }
    }
    ...
    return () => {
      if (video) {
        setTime(data._id, video.currentTime)
  ...
  return (
    <li className="relative grid grid-cols-subgrid col-span-full h-screen/1.6">
      <Link
        ...
        onClick={() => {
          if (videoRef.current) {
            setTime(data._id, videoRef.current.currentTime)
          }
        }}
                ...
                <div className="relative overflow-hidden">
                  <Heading
                    ...
                    dangerouslySetInnerHTML={{
                      __html: data?.musicalObject?.artist,
                    }}
